...
const MENU_PROJECTS = [
  ...
  { id: 'piikai', label: 'Piik', href: '/projects/piik' },
...
export function MobileQuickNav() {
  ...
  const menuGlass = isDark ? 'border-white/15 bg-black/60' : 'border-black/10 bg-white/80'
        ...
        <AnimatePresence>
          {open && (
            <motion.nav
              ...
              className={`pointer-events-auto flex min-w-[200px] max-w-[min(calc(100vw-2rem),280px)] flex-col overflow-hidden rounded-[18px] border shadow-xl backdrop-blur-xl ${menuGlass}`}
            >
              {MENU_PROJECTS.map((p) => (
                <Link
                  key={p.id}
                  ...
                  to={p.href}
                  onClick={close}
                  className={`flex items-center gap-3 px-4 py-2.5 font-mono text-[13px] font-semibold ${rowHover}`}
                >
