/**
 * Previous production homepage — backup at `/test-home-classic`.
 * Nested project overlays: `/test-home-classic/hovr`, `/test-home-classic/piik-ai`.
 */
...
export type HomePageExperienceConfig = {
   ...
   * Edit `HOME_PAGE_PRODUCTION_CONFIG` below to try `false` and other flags without touching `/`.
   */
  classicShellAndIntroColumn?: boolean
}

/** Isolated clone of production `/` — unique session keys so edits never touch live home storage. */
...
const DETAILS_GALLERY_THUMBS = [
  ...
  },
  {
    ...
    title: 'Piik AI',
    path: '/piik-ai',
    thumbnail_light: '/piikai/Thumbnail-light-sq.png',
    thumbnail_dark: '/piikai/Thumbnail-dark-sq.png',
  ...
  },
  {
    ...
    thumbnail_light: '/arfittingroom/Thumbnail-light-sq.png',
    thumbnail_dark: '/arfittingroom/Thumbnail-dark-sq.png',
  ...
  },
  {
    id: 'jojo',
    title: 'JoJo',
    thumbnail_light: '/jojo/Thumbnail-light.jpg',
    thumbnail_dark: '/jojo/Thumbnail-dark.jpg',
    tags: ['UX Research', 'AI Companion'],
    span: 'full' as const,
  },
] as const

const UNDER_CONSTRUCTION_GALLERY_IDS = new Set(['ar-fitting-room', 'jojo'])
...
function DetailsGalleryCard({
  ...
  project: (typeof DETAILS_GALLERY_THUMBS)[number]
  active: boolean
  onSelect: () => void
}) {
  const isSquare = 'square' in project && project.square
  const thumbClass = isSquare
  ...
  return (
    <button
      ...
      className="group relative block w-full cursor-pointer scroll-mt-4 overflow-hidden rounded-none bg-transparent p-0 text-left outline-none leading-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-black/40 dark:focus-visible:ring-white/40"
      style={{ borderRadius: 0 }}
    >
      <div className="relative block w-full leading-none overflow-hidden rounded-none" style={{ borderRadius: 0 }}>
        <img
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 bg-gradient-to-b from-black/55 via-black/25 to-transparent px-3 pb-14 pt-3 md:px-4">
        <div
          className={`grid max-w-full grid-cols-2 ${HOME_SUIT} text-[12pt] font-medium uppercase leading-4 tracking-[-0.02em] text-white`}
        >
          <div className="mb-1 leading-tight">{project.title}</div>
  ...
  return (
    <div className="flex w-full min-w-0 flex-col pb-4 md:min-h-0">
      <div className="grid w-full grid-cols-1 gap-1 md:grid-cols-2">
        {DETAILS_GALLERY_THUMBS.map((project) => (
          <div
            key={project.id}
        ...
        >
          <div className={`flex w-full flex-col pt-[2px] pb-[20px] ${foldBodyMuted}`}>
            ...
            >
              ...
              <span className="min-w-0 font-normal tracking-[0.04em]">READ HOW I MADE THIS WEBSITE</span>
              <span className="min-w-0 opacity-90">LINKEDIN</span>
            </a>
          </div>
        </ClassicFoldSection>
...
export function HomePageView({ config }: { config: HomePageExperienceConfig }) {
  ...
  return (
    ...
    >
      <HomePageViewInner config={config} />
    </ProjectListHoverPreviewProvider>
  )
}

function HomePageViewInner({ config }: { config: HomePageExperienceConfig }) {
  const navigate = useNavigate()
  const { pathname } = useLocation()
  const normalizedPath = pathname.replace(/\/+$/, '') || '/'
  const projectPopupOpen = normalizedPath !== '/'
      ...
      return (
        <DetailsThumbnailGallery
          ...
          onSelectProject={(id) => {
            ...
            const thumb = DETAILS_GALLERY_THUMBS.find((p) => p.id === id)
...
/** Backup of the previous production home — `/test-home-classic`. */
export function HomePageClassic() {
  return (
    <>
      <HomePageView config={HOME_PAGE_PRODUCTION_CONFIG} />
      <Outlet />
    </>
  )
}
